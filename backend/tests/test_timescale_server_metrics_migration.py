...
CONVERSION_SQL = (
    BACKEND_ROOT / "migrations/timescale/001_server_metrics_hypertable.sql"
).read_text(encoding="utf-8")
RETENTION_SQL = (
    BACKEND_ROOT / "migrations/timescale/002_server_metrics_retention.sql"
).read_text(encoding="utf-8")
...
def test_server_metric_orm_uses_timescale_compatible_composite_key():
    assert list(ServerMetric.__table__.primary_key.columns.keys()) == ["id", "time"]


def test_conversion_is_server_metrics_only_and_migrates_existing_rows():
    sql = normalized(CONVERSION_SQL)

    assert "create_hypertable" in sql
    assert "'server_metrics'" in sql
...
def test_conversion_has_atomic_before_after_invariant_guard():
    ...
    assert "raise exception 'server_metrics invariant mismatch" in sql
...
def test_fresh_install_schema_matches_server_metric_hypertable_contract():
    sql = normalized(INIT_SQL)

    assert "create extension if not exists timescaledb" in sql
    assert "primary key (id, time)" in sql
    assert "create_hypertable( 'server_metrics', 'time'" in sql
    assert "add_retention_policy( 'server_metrics', interval '30 days'" in sql
