...
CONVERSION_SQL = (
    BACKEND_ROOT / "migrations/timescale/005_disk_metrics_hypertable.sql"
).read_text(encoding="utf-8")
RETENTION_SQL = (
    BACKEND_ROOT / "migrations/timescale/006_disk_metrics_retention.sql"
).read_text(encoding="utf-8")
...
def test_disk_metric_orm_uses_timescale_compatible_composite_key():
    assert list(DiskMetric.__table__.primary_key.columns.keys()) == ["id", "time"]


def test_conversion_is_disk_metrics_only_and_migrates_existing_rows():
    sql = normalized(CONVERSION_SQL)

    assert "create_hypertable" in sql
    assert "'disk_metrics'" in sql
    assert "migrate_data => true" in sql
    assert "primary key (id, time)" in sql
    assert "server_metrics" not in sql
    assert "container_metrics" not in sql
...
def test_conversion_has_atomic_before_after_invariant_guard():
    ...
    assert "raise exception 'disk_metrics invariant mismatch" in sql
...
def test_fresh_install_schema_matches_disk_metric_hypertable_contract():
    sql = normalized(INIT_SQL)
    disk_section = sql.split("-- disk metrics", 1)[1].split("-- alert rules", 1)[0]

    assert "create extension if not exists timescaledb" in sql
    assert "create table if not exists disk_metrics" in disk_section
    assert "primary key (id, time)" in disk_section
    assert "create_hypertable( 'disk_metrics', 'time'" in disk_section
    assert (
        "add_retention_policy( 'disk_metrics', interval '30 days'"
        in disk_section
    )
